  ...
  for (const location of data) {
    ...
    location.forEach((climb) => {
      ...
      for (let item of feature.geometry.coordinates) {
        //console.log(item)
        if (
          item[1] == climb.coordinates[1] &&
          item[0] == climb.coordinates[0]
        ) {
          inArrayTrigger = true;
          break;
        }
      ...
      if (!climbExists) {
        feature.properties.climbs.push({
          id: climb.id,
          climber_names: climb.climber_names,
          name: climb.name,
          grade: climb.grade,
        });
      }
    ...
    if (feature.geometry.coordinates.length === 1) {
      ...
      const pointsCollection = turf.featureCollection(
        feature.geometry.coordinates.map((coord) => turf.point(coord)),
      );
